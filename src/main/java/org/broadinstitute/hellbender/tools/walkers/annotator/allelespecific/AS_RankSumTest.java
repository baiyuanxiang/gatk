...
//            return Arrays.asList(GATKVCFHeaderLines.getInfoLine(getKeyNames().get(0)));
     ...
     * given it's best assigned allele and returns the value of the allele as a double. This data gets condensed into a
     * CompressedDataList object represented as a string and returns this in a map with its key being the name of the
     * raw annotation.
     *
    ...
    //TODO figure out if this needs to be a double to behave correctly
    ...
    /**
     * Unless overridden by client methods, parses the raw data histograms generated for each allele and computes the
     * median value for each allele and outputs that as the representative double value.
     *
    ...
    /**
     * Parses expected raw data strings per-allel, which are in the form of run length encoded histograms, and overlays them by
     * creating a new histogram which represents the combination of all elements in each combined histogram into one
     * data source.
     *
     * @param vcAlleles -- List of alleles over which to calculate RankSum for
     * @param annotationList -- Annotation
     * @return Single element with the annotation name as its key, and a '|' separated list of run lenght encoded histograms
